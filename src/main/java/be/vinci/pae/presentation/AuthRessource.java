...
public class AuthRessource {
   ...
   *
   * @param json    the oldPassword and the newPassword
   * @param request the request
   * @return a message
   */
  @POST
  ...
  public Response changePassword(JsonNode json, @Context ContainerRequest request) {
    ...
    if (json.get("newPassword").asText().equals(json.get("confirmationPassword").asText())
        && json.get("newPassword").asText().equals(json.get("oldPassword").asText())) {
      throw new WebApplicationException("New password is the same as the old one",
          Response.Status.BAD_REQUEST);
    }

    if (!json.get("newPassword").asText().equals(json.get("confirmationPassword").asText())
        || json.get("newPassword").asText().isEmpty() || json.get("confirmationPassword").asText()
        .isEmpty()) {
    ...
    if (json.get("oldPassword").asText().isEmpty() || json.get("newPassword").asText().isEmpty()
        || json
        .get("confirmationPassword").asText().isEmpty()) {
    ...
    if (!userUCC.changePassword(authenticatedUser.getIdUser(), oldPassword, newPassword)) {
